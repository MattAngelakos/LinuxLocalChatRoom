...
do_register(State, Ref, ClientPID, ClientNick) ->
    NewRegistrations = maps:put(ClientPID, ClientNick, State#chat_st.registrations),
	NewState = State#chat_st{registrations = NewRegistrations},
	ClientPID!{self(), Ref, connect, State#chat_st.history},
	NewState.

%% This function should unregister a client from this chatroom
do_unregister(State, ClientPID) ->
    NewRegistrations = maps:remove(ClientPID, State#chat_st.registrations),
	NewState = State#chat_st{registrations = NewRegistrations},
	NewState.

%% This function should update the nickname of specified client.
do_update_nick(State, ClientPID, NewNick) ->
	NewRegistrations = maps:update(ClientPID, NewNick, State#chat_st.registrations),
    NewState = State#chat_st{registrations = NewRegistrations},
	NewState.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	lists:foreach(fun(CurrClientPID) ->
		case CurrClientPID == ClientPID of
			true ->
				ok;
			false ->
				CurrClientPID!{request, self(), Ref, {incoming_msg, CliNick, State#chat_st.name, Message}}
		end
    end, maps:keys(State#chat_st.registrations)),
	NewHistory = State#chat_st.history ++ [{CliNick, Message}],
	NewState = State#chat_st{history = NewHistory},
	NewState.
